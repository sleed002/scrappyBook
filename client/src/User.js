...
class User extends React.Component {
        ...
        <div className="Container">
          <div className="row">
            ...
            <div className="col-md-8">
              <Button className="btn btn-sm btn-outline-secondary" onClick={() => this.handleDelete()}>Delete a user</Button>
              <Link to={`/users/${users.user_id}/edit`}>
                <Button className="btn btn-sm btn-outline-secondary">
                  Edit</Button>
              </Link>
              <Link to={`/users/${users.user_id}/posts/new`}>
                <Button className="btn btn-sm btn-outline-secondary">
  ...
  renderPhotos(userid, photo, postid) {
    ...
    if (postid === photoPostId) {
      return (photo.photoarr.map(photoUrl => this.renderPhoto(userid, postid, photoUrl)));
    }
  }

  renderPhoto(userid, postid, photoUrl) {
    return (<span key={photoUrl}>
